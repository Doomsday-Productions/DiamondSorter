...
from PyQt5.QtGui import QDesktopServices, QTextCursor, QTextDocument, QColor, QCursor, QTextCharFormat, QIcon, QPainter, QTextOption, QFont
import binascii
...
try:
    ...
    try:
        ...
        print("⚠️  ui_form not found - some UI features may not work")
        ui_form = None 
import requests
import time
import curses
...
try:
    ...
    print("⚠️  flask not available - some features may not work")
import warnings
...
try:
    ...
    print("⚠️  tkinter not available - some features may not work")
...
class DiamondSorter(*top_classes):
    ...
    def __init__(self, directory_path=None, input_textedit=None):
        ...
        # Initialize zoom level (1.0 = 100%, 1.2 = 120%, etc.)
        self.zoom_level = 1.0
        self.min_zoom = 0.5
        self.max_zoom = 3.0
        self.zoom_step = 0.1
        ...
        if INSTALLER_MODE:
            ...
            # Try to find form.ui in different locations
            ui_paths = [
                'form.ui',
                'src/ui/form.ui',
                os.path.join(os.path.dirname(__file__), '..', 'ui', 'form.ui'),
            ...
            if ui_file:
                ...
                print("⚠️  form.ui not found - please ensure form.ui is in the correct location")
        ...
        # Set responsive window sizing to fit different screen resolutions
        ...
        # Add scrolling keyboard shortcuts
        self.scroll_to_top_shortcut = QShortcut(QKeySequence("Ctrl+Home"), self, context=Qt.WidgetShortcut)
        self.scroll_to_top_shortcut.activated.connect(self.scroll_to_top)
        
        self.scroll_to_bottom_shortcut = QShortcut(QKeySequence("Ctrl+End"), self, context=Qt.WidgetShortcut)
        self.scroll_to_bottom_shortcut.activated.connect(self.scroll_to_bottom)
        
        # Add zoom keyboard shortcuts
        self.zoom_in_shortcut = QShortcut(QKeySequence("Ctrl+="), self, context=Qt.WidgetShortcut)
        self.zoom_in_shortcut.activated.connect(self.zoom_in)
        
        self.zoom_out_shortcut = QShortcut(QKeySequence("Ctrl+-"), self, context=Qt.WidgetShortcut)
        self.zoom_out_shortcut.activated.connect(self.zoom_out)
        
        # Add Ctrl+0 to reset zoom
        self.zoom_reset_shortcut = QShortcut(QKeySequence("Ctrl+0"), self, context=Qt.WidgetShortcut)
        ...
        # Store the original stylesheet before applying any zoom
        self.original_stylesheet = self.styleSheet()
        
        # Add zoom actions to the far right of the menu bar
        self.add_zoom_buttons_to_menu()
        
        # Initialize zoom level
        self.apply_zoom()

    def setup_responsive_window(self):
        """Set up responsive window sizing to fit different screen resolutions."""
        try:
            ...
            # For smaller screens (like laptops), use a smaller percentage
            if screen_height <= 768:  # Small laptop screens
                window_width = min(1000, int(screen_width * 0.9))
                window_height = min(600, int(screen_height * 0.85))
            elif screen_height <= 900:  # Medium laptop screens
            ...
            window_width = min(window_width, screen_width - 50)  # Leave 50px margin
            window_height = min(window_height, screen_height - 50)  # Leave 50px margin
            ...
            # Refresh scroll area after a short delay to ensure all UI elements are loaded
            QTimer.singleShot(100, self.refresh_scroll_area)
            
            # Connect to resize event to ensure window stays within bounds
            self.resizeEvent = self.handle_resize_event
            
            print(f"Window sized to {window_width}x{window_height} on screen {screen_width}x{screen_height}")
    ...
    def handle_resize_event(self, event):
        """Handle window resize events to ensure window stays within screen bounds."""
        try:
            ...
            # Check if window is outside screen bounds and adjust if necessary
            new_x = current_geometry.x()
            new_y = current_geometry.y()
            new_width = current_geometry.width()
            new_height = current_geometry.height()
            
            # Adjust if window goes off the right edge
            if current_geometry.right() > screen_width:
                new_x = screen_width - new_width - 10  # 10px margin
                
            # Adjust if window goes off the bottom edge
            if current_geometry.bottom() > screen_height:
                new_y = screen_height - new_height - 10  # 10px margin
                
            # Adjust if window goes off the left edge
            if current_geometry.left() < 0:
                new_x = 10  # 10px margin
                
            # Adjust if window goes off the top edge
            if current_geometry.top() < 0:
                new_y = 10  # 10px margin
            ...
            # Call parent resize event even if there's an error
            super().resizeEvent(event)

    def setup_scrolling(self):
        """Set up scrolling functionality for the main window content."""
        try:
            # Get the central widget
            central_widget = self.centralWidget()
            if central_widget is None:
                print("Warning: No central widget found for scrolling setup")
                return
            
            # Create a scroll area
            scroll_area = QScrollArea()
            scroll_area.setWidget(central_widget)
            scroll_area.setWidgetResizable(False)  # Allow widget to maintain its natural size
            
            # Enable both horizontal and vertical scrolling with always visible scrollbars
            scroll_area.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOn)
            scroll_area.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOn)
            
            # Set scroll area as the new central widget
            ...
            # Ensure the widget maintains its size and scrollbars work properly
            central_widget.setMinimumSize(central_widget.sizeHint())
            
            # Configure scrollbar appearance and behavior
            scroll_area.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOn)
            scroll_area.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOn)
            
            # Force update to ensure scrollbars are properly calculated
            scroll_area.updateGeometry()
            central_widget.updateGeometry()
            
            # Style the scrollbars for better visibility
            self.style_scrollbars(scroll_area)
            
            # Store reference to scroll area for later use
            self.scroll_area = scroll_area
            
            print("Scrolling functionality enabled")
            
        except Exception as e:
            print(f"Error setting up scrolling: {e}")

    def refresh_scroll_area(self):
        """Refresh the scroll area to recalculate content size and scrollbars."""
        if hasattr(self, 'scroll_area') and self.scroll_area:
            try:
                # Force the scroll area to recalculate its content
                self.scroll_area.updateGeometry()
                if self.scroll_area.widget():
                    self.scroll_area.widget().updateGeometry()
                    self.scroll_area.widget().adjustSize()
                self.scroll_area.update()
                print("Scroll area refreshed")
            except Exception as e:
                print(f"Error refreshing scroll area: {e}")

    def style_scrollbars(self, scroll_area):
        """Style the scrollbars for better visibility and user experience."""
        try:
            # Get the scrollbars
            v_scrollbar = scroll_area.verticalScrollBar()
            h_scrollbar = scroll_area.horizontalScrollBar()
            
            # Set scrollbar properties for better visibility
            if v_scrollbar:
            ...
            print("Scrollbars styled for better visibility")
            
        except Exception as e:
            print(f"Error styling scrollbars: {e}")
    ...
    def scroll_to_widget(self, widget):
        """Scroll to a specific widget within the scroll area."""
        if hasattr(self, 'scroll_area') and self.scroll_area and widget:
            self.scroll_area.ensureWidgetVisible(widget)

    def enable_auto_scroll(self, enabled=True):
        """Enable or disable auto-scrolling behavior."""
        if hasattr(self, 'scroll_area') and self.scroll_area:
            if enabled:
                # Enable always visible scrollbars
    ...
    def add_zoom_buttons_to_menu(self):
        """Add zoom buttons to the far right of the menu bar"""
        menu_bar = self.menuBar()
        ...
        menu_bar.addAction(zoom_in_action)
        
        zoom_out_action = QAction("🔍-", self)
        zoom_out_action.setToolTip("Zoom Out (Ctrl+-)")
        zoom_out_action.triggered.connect(self.zoom_out)
        menu_bar.addAction(zoom_out_action)

    def zoom_in(self):
        """Increase the zoom level of the application"""
    ...
    def zoom_out(self):
        """Decrease the zoom level of the application"""
        if self.zoom_level > self.min_zoom:
            self.zoom_level = max(self.zoom_level - self.zoom_step, self.min_zoom)
            self.apply_zoom()

    def apply_zoom(self):
        """Apply the current zoom level to all UI elements"""
        # Calculate new font size based on zoom level
        base_font_size = 9
        new_size = int(base_font_size * self.zoom_level)
        
        # Applying zoom functionality
        
        # Create a new font with the scaled size
        font = QFont()
        font.setPointSize(new_size)
        
        # Apply font to the main window and recursively to all child widgets
        self.setFont(font)
        self.apply_font_to_children(self, font)
        
        # Apply stylesheet-based scaling for better visual effect
        self.apply_stylesheet_zoom()
        
        # Update the window title to show current zoom level
        base_title = "Diamond Sorter"
        if hasattr(self, 'original_title'):
            base_title = self.original_title
        else:
            self.original_title = base_title
        
        zoom_percentage = int(self.zoom_level * 100)
        self.setWindowTitle(f"{base_title} - {zoom_percentage}%")
        
        # Force a complete refresh of all widgets
    ...
    def refresh_all_widgets(self):
        """Force refresh of all widgets in the application"""
        # Get all widgets and force them to update
        ...
        for widget in all_widgets:
            ...
            if hasattr(widget, 'setFont'):
                font = widget.font()
                font.setPointSize(int(9 * self.zoom_level))
                widget.setFont(font)

    def apply_stylesheet_zoom(self):
        """Apply stylesheet-based zoom scaling without overriding theme colors"""
        # Create a stylesheet that only scales font sizes without affecting colors
        font_size = int(9 * self.zoom_level)
        
        # Minimal stylesheet that only affects font sizes
        zoom_stylesheet = f"""
        QWidget {{
        ...
        QPushButton {{
            font-size: {font_size}px;
            padding: {int(4 * self.zoom_level)}px;
            min-height: {int(20 * self.zoom_level)}px;
        }}
        ...
        QTabBar::tab {{
            font-size: {font_size}px;
            padding: {int(6 * self.zoom_level)}px {int(12 * self.zoom_level)}px;
        }}
        QMenuBar {{
            font-size: {font_size}px;
        }}
        ...
        if not hasattr(self, 'original_stylesheet'):
            self.original_stylesheet = self.styleSheet()
        
        # Combine original stylesheet with zoom stylesheet
        combined_stylesheet = self.original_stylesheet + "\n" + zoom_stylesheet
        
        # Apply the combined stylesheet to the main window
        self.setStyleSheet(combined_stylesheet)
        # Applied zoom stylesheet

    def apply_font_to_children(self, widget, font):
        """Recursively apply font to all child widgets"""
        for child in widget.findChildren(QWidget):
            if child != widget:  # Don't apply to self
                child.setFont(font)
                # Also update any QTextEdit, QTextBrowser, QLabel, etc. specifically
    ...
    def reset_zoom(self):
        ...
        self.zoom_level = 1.0
        # Restore original stylesheet when resetting to 100%
        if hasattr(self, 'original_stylesheet'):
...
# Function to add option dialog to select a specific country phone code for all countries
    ...
    def setup_buttons(self):
        ...
        self.widget_button_url_tools.clicked.connect(self.launch_url_manager)
        ...
        # self.recovery_key_button.clicked.connect(self.handle_recovery_key_button_click)  # Widget not found in UI
        self.eth_private_button = QPushButton("ETH Private Key")
        self.eth_private_button.clicked.connect(self.eth_private_button_clicked)
        self.wallet_import_format_button = QPushButton("Wallet Import Format")
        self.wallet_import_format_button.clicked.connect(self.wallet_import_format_button_clicked)
        self.recovery_phrase_button = QPushButton("Recovery Phrase")
        self.recovery_phrase_button.clicked.connect(self.recovery_phrase_button_clicked)
        ...
        self.remove_before_button_2 = QPushButton("Remove Before Button 2")
        ...
        self.copy_results_button = QPushButton("Copy Results")
    ...
    def eth_private_button_clicked(self):
        """Handle ETH Private Key button click."""
        try:
            input_text = self.findChild(QTextEdit, "input_text")
            output_text = self.findChild(QTextEdit, "output_text")
            if input_text is not None and output_text is not None:
                text = input_text.toPlainText()
                # Extract Ethereum private keys (64 character hex strings)
                eth_private_key_pattern = r'\b[a-fA-F0-9]{64}\b'
                eth_keys = re.findall(eth_private_key_pattern, text)
                
                output_text.clear()
                output_text.setPlainText('\n'.join(eth_keys))
                
                # Remove found keys from input
                remaining_text = re.sub(eth_private_key_pattern, '', text)
                input_text.setPlainText(remaining_text)
                
                QMessageBox.information(self, "ETH Private Keys", f"Found {len(eth_keys)} Ethereum private keys.")
        except Exception as e:
            print(f"An error occurred: {e}")

    def wallet_import_format_button_clicked(self):
        """Handle Wallet Import Format button click."""
        try:
            input_text = self.findChild(QTextEdit, "input_text")
            output_text = self.findChild(QTextEdit, "output_text")
            if input_text is not None and output_text is not None:
                text = input_text.toPlainText()
                # Extract wallet import format (WIF) - typically 51-52 characters starting with 5, K, or L
                ...
                # Remove found WIF keys from input
                remaining_text = re.sub(wif_pattern, '', text)
                input_text.setPlainText(remaining_text)
                
                QMessageBox.information(self, "Wallet Import Format", f"Found {len(wif_keys)} WIF keys.")
        except Exception as e:
            print(f"An error occurred: {e}")

    def recovery_phrase_button_clicked(self):
        """Handle Recovery Phrase button click."""
        try:
            input_text = self.findChild(QTextEdit, "input_text")
            output_text = self.findChild(QTextEdit, "output_text")
            if input_text is not None and output_text is not None:
                text = input_text.toPlainText()
                # Extract recovery phrases (12, 15, 18, 21, or 24 words)
                # Common BIP39 word patterns
                recovery_phrase_pattern = r'\b(?:[a-z]+\s+){11,23}[a-z]+\b'
                phrases = re.findall(recovery_phrase_pattern, text)
                
                # Filter for common word counts
                valid_phrases = []
                ...
                QMessageBox.information(self, "Recovery Phrases", f"Found {len(valid_phrases)} recovery phrases.")
        except Exception as e:
            print(f"An error occurred: {e}")

    def remove_before_button_2_function(self):
        """Handle Remove Before Button 2 click."""
        try:
            input_text = self.findChild(QTextEdit, "input_text")
            output_text = self.findChild(QTextEdit, "output_text")
            if input_text is not None and output_text is not None:
                ...
                for line in lines:
                    if " " in line:
                        ...
                        processed_line = parts[1] if len(parts) > 1 else line
                        removed_part = parts[0] if len(parts) > 1 else ""
                    else:
                        processed_line = line
                        removed_part = ""
                ...
                removed_data_text = self.findChild(QTextEdit, "removed_data_text")
                if removed_data_text is not None:
                    removed_data_text.clear()
                    removed_data_text.setPlainText("\n".join(removed_data))
                
                QMessageBox.information(self, "Remove Before", "Text before first space has been removed.")
        except Exception as e:
            print(f"An error occurred: {e}")

    def reformat_button_function(self):
        """Handle Reformat button click."""
        try:
            input_text = self.findChild(QTextEdit, "input_text")
            output_text = self.findChild(QTextEdit, "output_text")
            if input_text is not None and output_text is not None:
                ...
                output_text.clear()
                output_text.setPlainText(formatted_text)
                
                QMessageBox.information(self, "Reformat", "Text has been reformatted (empty lines removed, whitespace trimmed).")
        except Exception as e:
            print(f"An error occurred: {e}")
