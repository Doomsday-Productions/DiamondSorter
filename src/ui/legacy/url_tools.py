...
"Requires a wordlist to be used"))
        ...
        self.token_textedit.setPlaceholderText(_translate("UrlToolsWindow", "Telegram Bot ID ( From Botfather )"))
        ...
        self.pushButton_3.setText(_translate("UrlToolsWindow", "Add http(s)://"))
        self.pushButton_4.setText(_translate("UrlToolsWindow", "Add /wp-login.php"))
        self.pushButton_5.setText(_translate("UrlToolsWindow", "Add /xmlrpc.php"))
        self.analytics_and_config_tab_widget.setTabText(self.analytics_and_config_tab_widget.indexOf(self.wordpress_tab), _translate("UrlToolsWindow", "WordPress & Woocommerce"))
        ...
        self.checkBox_3 = QCheckBox()  # Instantiate QCheckBox widget
        self.checkBox_3.stateChanged.connect(self.toggle_word_wrap)
        self.forward_messages_button.clicked.connect(self.forward_messages_button_clicked)

    def forward_messages_button_clicked(self):
        token = self.token_textedit.toPlainText()
        chat_id = self.telegram_chatID_bottools.text()
        content = self.token_textedit.toPlainText()
    
        if token and chat_id:
            bot_token = "6329154697:AAF-XkmfyKgrAB9VVGPBkHFZekVQrbwHapk"
            ...
            if response.status_code == 200:
                self.results_window_textedit_4.setPlainText("Content forwarded successfully.")
            else:
                self.results_window_textedit_4.setPlainText("Error: Failed to forward content.")
        else:
            self.results_window_textedit_4.setPlainText("Error: Invalid token or chat ID.")
    
    def toggle_word_wrap(self, state):
        if state == Qt.Checked:
            self.results_window_textedit_4.setWordWrapMode(QTextEdit.WordWrap)
            self.results_window_textedit_4.append("Word wrap is enabled.")
        else:
            self.results_window_textedit_4.setWordWrapMode(QTextEdit.NoWrap)
            self.results_window_textedit_4.append("Word wrap is disabled.")
    ...
    def get_updates(self):
        ...
        if token:
            ...
            error_dialog.setIcon(QMessageBox.Critical)
            error_dialog.setWindowTitle("Error")
            error_dialog.setText("Please enter a valid token.")
            error_dialog.exec_()
